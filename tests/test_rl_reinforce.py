...
def test_reinforce_discrete():
    ...
    trainer = RawTrainer(env, agent, NaiveMemory(), 500, batch_size=999, render=False, verbose=1)
    trainer.train()
    trainer.evaluation()
...
def test_reinforce_continuous():
    ...
    device = torch.device("cpu")
    print(device)
    env = PendulumEnvWrapper(gym.make("Pendulum-v0"))
    env.seed(0)
    ...
    trainer = RawTrainer(env, agent, NaiveMemory(), episode=500, learn_freq=9999, batch_size=9999, render=False,
                         verbose=1)
    trainer.train()
